...
function Loai({ history, permission }) {
  ...
  const getListData = (keyword, page, pageSize) => {
    ...
    dispatch(fetchStart(`Loai?${param}`, "GET", null, "LIST"));
  };
  /**
   * handleTableChange
   *
  ...
  const deleteItemAction = (item) => {
    let url = `Loai/${item.id}`;
    if (item.isRemove) url = `Loai/Remove/${item.id}`;
  ...
  const actionContent = (item) => {
    const editItem = permission.edit ? (
      <Link
        to={{
          pathname: `/danh-muc/loai/${item.id}/chinh-sua`,
  ...
  let colValues = [
    ...
    { title: "Mã loại", dataIndex: "maLoai", key: "maLoai", width: "40%" },
    { title: "Tên loại", dataIndex: "tenLoai", key: "tenLoai", width: "40%" },
    {
      title: "Hành động",
      key: "action",
      align: "center",
  ...
  const handleRedirect = () => {
    history.push({
      pathname: "/danh-muc/loai/them-moi",
    });
  };
