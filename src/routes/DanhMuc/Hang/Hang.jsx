...
function Hang({ history, permission }) {
  ...
  const getListData = (keyword, page, pageSize) => {
    ...
    dispatch(fetchStart(`Hang?${param}`, "GET", null, "LIST"));
  };
  /**
   * handleTableChange
   *
  ...
  const deleteItemAction = (item) => {
    let url = `Hang/${item.id}`;
    if (item.isRemove) url = `Hang/Remove/${item.id}`;
  ...
  const actionContent = (item) => {
    const editItem = permission.edit ? (
      <Link
        to={{
          pathname: `/danh-muc/hang/${item.id}/chinh-sua`,
  ...
  let colValues = [
    ...
    { title: "Mã hãng", dataIndex: "maHang", key: "maHang" },
    { title: "Tên hãng", dataIndex: "tenHang", key: "tenHang" },
    {
      title: "Hành động",
      key: "action",
      align: "center",
  ...
  const handleRedirect = () => {
    history.push({
      pathname: "/danh-muc/hang/them-moi",
    });
  };
